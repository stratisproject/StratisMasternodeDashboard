...
namespace Stratis.FederatedSidechains.AdminDashboard.Controllers
{
    ...
    public class StratisNodeController : Controller
    {
        ...
        public StratisNodeController(DefaultEndpointsSettings defaultEndpointsSettings, ApiRequester apiRequester)
        {
        ...
        public async Task<IActionResult> ResyncAsync(string value)
        {
            ...
            if (isHeight)
            {
                ApiResponse getblockhashRequest = await this.apiRequester.GetRequestAsync(this.defaultEndpointsSettings.MainchainNodeEndpoint, $"/api/Consensus/getblockhash?height={value}");
                ApiResponse syncRequest = await this.apiRequester.PostRequestAsync(this.defaultEndpointsSettings.MainchainNodeEndpoint, "/api/Wallet/sync", new { hash = ((string)getblockhashRequest.Content) });
                return syncRequest.IsSuccess ? (IActionResult)Ok() : BadRequest();
            }
            else
            {
                ApiResponse syncRequest = await this.apiRequester.PostRequestAsync(this.defaultEndpointsSettings.MainchainNodeEndpoint, "/api/Wallet/sync", new { hash = value });
                return syncRequest.IsSuccess ? (IActionResult)Ok() : BadRequest();
            }
        }
        ...
        {
            ApiResponse stopNodeRequest = await this.apiRequester.PostRequestAsync(this.defaultEndpointsSettings.MainchainNodeEndpoint, "/api/Node/stop", true);
            return stopNodeRequest.IsSuccess ? (IActionResult)Ok() : BadRequest();
        }
        ...
        {
            ApiResponse changeLogLevelRequest = await this.apiRequester.PostRequestAsync(this.defaultEndpointsSettings.MainchainNodeEndpoint, "/api/Node/loglevels", new { logRules = new[] { new { ruleName = rule, logLevel = level } } }, Method.PUT);
